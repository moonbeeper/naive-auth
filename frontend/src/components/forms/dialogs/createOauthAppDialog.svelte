...
<script lang="ts">
	...
	}: { data: (data: components['schemas']['CreateAppResponse']) => void; open: boolean } = $props();
	...
	const form = superForm(defaults(zod4(schema)), {
		...
		onUpdate: async ({ form: f }) => {
			if (f.valid) {
				const res = await client.POST('/v1/oauth/apps', {
					body: {
				...
				if (res.error?.error === ('OAuthInvalidUri' as ApiHttpError)) {
					setError(f, 'callback_url', res.error.message);
				} else if (res.error) {
				...
				{#if open}
					<div {...props} in:scale={{ duration: 200 }}>
						<div class="dialog-header">
							<Dialog.Title>App information</Dialog.Title>
							<Dialog.Close>
								{#snippet child({ props })}
