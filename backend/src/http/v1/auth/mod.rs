...
// ///
// /// This will verify the email address of a user, allowing them to login with password. The email can be also be
// /// verified by using the OTP authentication method.
// #[utoipa::path(
//     post,
//     path = "/verify/{id}",
...
//     Valid(Path(request_path)): Valid<Path<VerifyEmailPath>>,
...
//     let Some(mut user) = User::get_by_email(&request_path.email, &global.database).await? else {
